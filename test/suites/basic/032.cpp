...
struct Test032 : Test {
	...
	inline Test032(const char * name, const char * desc, runner func) noexcept
	  : Test(name, desc, func) {}
...
Test032 Test032::tests[] = {
	...
	RUN("parsing array: uchar[3] wrong end in mismatched", {
		return arr<unsigned char COMMA 3>::run(env,
				"[1, [[]}], 3]", {1, 0, 0}, error_t::mismatch | error_t::bad ); }),
	RUN("parsing array: long[3]", {
		return arr<long COMMA 3>::run2(env,"[\n\t1,\n\t2,\n\t3\n]", {1, 2, 3}); }),
};
