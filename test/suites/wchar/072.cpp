 ...
 *
 * 072.cpp - cojson tests, read/write with char32_t
 *
...
namespace test {
template<>
bool HostEnvironment::put<char32_t>(char_t chr, FILE* out) const noexcept {
	/* combination of char32_t char_t used to catch misconfiguration for
	 * this test															 */
	char mb[MB_CUR_MAX];
	mbstate_t ps {0, 0};
...
struct Test072 : Test {
	static Test072 tests[];
	inline Test072(const char * name, const char * desc, runner func)
	  : Test(name, desc,func) {
...
#define COMMA ,
#define RUN(name, body) Test072(__FILE__,name, \
		[](const Environment& env) noexcept -> result_t body)
Test072 Test072::tests[] = {
	RUN("POD object", {
...
#undef  _T_
#define _T_ (7200)
static const master_t Master[std::extent<decltype(Test072::tests)>::value] = {
...
const master_t Test072::master() const noexcept {
	return Master[index()];
}
}}
