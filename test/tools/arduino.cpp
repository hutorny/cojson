...
template<>
char master_t::get<char>(unsigned n) {
	return pgm_read_byte(farptr()+n);
}
...
struct DefaultEnvironment : McuEnvironment {
	...
	int match(master_t master) const noexcept {
		return master.data ? ! memcmp_PF(
			output.begin(), master.farptr(), output.count()*sizeof(char_t))
			: !0;
	}
	...
	void msg(verbosity lvl, master_t piece) const noexcept {
		if( lvl > options.level ) return;
		char chr;
		for(unsigned i = 0; (chr = piece.get<char>(i)); ++i) {
			write(chr);
		}
		write('\n');
	}
...
template<> const char pstring<0>::str[] __attribute__((progmem)) =
	"cojson test runner\n"
...
static void print_help() {
	environment.msg(LVL::silent, master_t{pstring<0>::str});
}
